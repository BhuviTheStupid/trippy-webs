...
const Fast = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0e0e0e] px-6 py-10 text-white pb-28 relative">
      <h1 className="text-4xl font-bold text-center mb-12 text-red-400 drop-shadow-glow">
        ⚡ Hyper Pulse: Fast Vibes
      </h1>
      ...
      {songs.length === 0 ? (
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {songs.map((song, index) => (
            <div
              key={song.id}
              className="bg-[#1a1a1a] hover:bg-[#292929] rounded-xl p-4 shadow-md hover:shadow-red-400/30 transition-all duration-300 group"
            >
              <img
                src={song.thumbnail_url || "/images/default-thumb.jpg"}
                alt={song.name}
                className="rounded-md h-48 w-full object-cover mb-3 border border-red-400/10"
              ...
              <div className="flex justify-between items-center mt-2">
                <div>
                  <h2 className="text-lg font-semibold text-red-200 truncate">
                    {song.name || "Untitled Track"}
                  </h2>
                  <p className="text-sm text-red-100 opacity-80 italic">by Beat Jumper</p>
                </div>
                <button
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-[#181818] border-t border-gray-800 px-4 py-3 flex items-center justify-between z-50 shadow-lg">
          <div className="flex items-center gap-4 overflow-hidden">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Fast Vibe • Beat Jumper</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handleNext}
              className="bg-yellow-500 hover:bg-yellow-400 text-black text-lg px-4 py-2 rounded-full shadow transition"
              title="Next random"
            >
              ⏭
            </button>
            <button
              ...
              className="bg-red-500 hover:bg-red-400 text-white font-bold text-lg px-4 py-2 rounded-full shadow transition-all"
            >

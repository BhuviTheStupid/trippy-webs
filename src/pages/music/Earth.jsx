...
const Earth = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0f0e0d] px-6 py-10 text-white pb-28 relative">
      <h1 className="text-4xl font-bold text-center mb-12 text-amber-400 drop-shadow-glow">
        🌌 Earth Core: Grounded Tones
      </h1>
      ...
      {songs.length === 0 ? (
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {songs.map((song, index) => (
            <div
              key={song.id}
              className="bg-[#1b1a17] hover:bg-[#222019] rounded-xl p-4 shadow-md hover:shadow-amber-500/30 transition-all duration-300 group"
            >
              <img
                src={song.thumbnail_url || "/images/default-thumb.jpg"}
                alt={song.name}
                className="rounded-md h-48 w-full object-cover mb-3 border border-amber-400/10"
              ...
              <div className="flex justify-between items-center mt-2">
                <div>
                  <h2 className="text-lg font-semibold text-amber-200 truncate">
                    {song.name || "Untitled Track"}
                  </h2>
                  <p className="text-sm text-amber-100 opacity-80 italic">by Earth Shaker</p>
                </div>
                <button
                  ...
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-xl font-bold transition-all duration-300 shadow-md border-2 ${
                    playing === index && !isPaused
                      ...
                      : "bg-amber-700 hover:bg-amber-600 border-amber-300 text-white"
      ...
      {showPlayer && playing !== null && (
        <div className="fixed bottom-0 left-0 right-0 bg-[#151411] border-t border-gray-800 px-4 py-3 flex items-center justify-between z-50 shadow-lg">
          <div className="flex items-center gap-4 overflow-hidden">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Earth Vibe • Earth Shaker</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handleNext}
              className="bg-green-700 hover:bg-green-600 text-white text-lg px-4 py-2 rounded-full shadow transition"
              title="Next random"
            >
              ⏭
            </button>
            <button
              ...
              className="bg-amber-500 hover:bg-amber-400 text-black font-bold text-lg px-4 py-2 rounded-full shadow transition-all"
            >
